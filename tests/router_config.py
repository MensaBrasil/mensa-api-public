...
async def test_auth_role_valid(
    token_data: UserToken = Depends(permission_required("CREATE.EVENT")),
):
    return {"message": "Authenticated with CREATE.EVENT permission"}


@test_router.get("/auth-role-invalid")
async def test_auth_role_invalid(
    token_data: UserToken = Depends(permission_required("NON_EXISTENT.PERMISSION")),
):
...
async def test_auth_group_valid(
    token_data: UserToken = Depends(permission_required("WHATSAPP.BOT")),
):
...
async def test_auth_group_invalid(
    token_data: UserToken = Depends(permission_required("EDIT.EVENT")),
):
