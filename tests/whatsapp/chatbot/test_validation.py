...
def test_chatbot_validation_member_not_found(test_client, sign_twilio_request):
    """
    ...
    headers = sign_twilio_request(url, payload)
    response = test_client.post("/whatsapp/chatbot-message", data=payload, headers=headers)
    assert response.status_code == 404
    assert response.json()["detail"] == "Member not found"


def test_chatbot_validation_payment_expired(test_client, sign_twilio_request):
    """
...
def test_chatbot_validation_member_active(test_client, sign_twilio_request):
    """
...
def test_chatbot_validation_invalid_signature(test_client):
    """
    Integration test for /whatsapp/chatbot-message with an invalid Twilio signature.
    """
    payload = {
        "SmsMessageSid": "SM123",
        "NumMedia": "0",
