...
from sqlmodel import SQLModel, Field, Column, Text, select, func, desc, col
from sqlalchemy.sql.elements import ColumnElement
from sqlmodel.sql.expression import Select
from people_api.database.models.models import Registration, BaseSQLModel
...
class BaseVolunteerActivityLog(BaseSQLModel):
    ...
    registration_id: Optional[int] = Field(default=None, foreign_key="registration.registration_id", exclude=True)
...
class VolunteerActivityLog(BaseVolunteerActivityLog, table=True):
    ...
    def select_unevaluated(cls):
        return (
            select(cls)
            .outerjoin(VolunteerActivityEvaluation, col(cls.id) == col(VolunteerActivityEvaluation.activity_id))
            .where(VolunteerActivityEvaluation.id == None)
        )
    ...
    def select_full_user_activities(cls, registration_id: int):
        ...
        stmt = (
            ...
            .outerjoin(VolunteerActivityEvaluation, col(VolunteerActivityEvaluation.activity_id) == col(cls.id))
...
class VolunteerActivityEvaluation(BaseVolunteerActivityEvaluation, table=True):
    ...
    def select_evaluated_activities_for_member(cls, registration_id: int):
        ...
        return (
            ...
            .join(VolunteerActivityLog, col(VolunteerActivityLog.id) == col(cls.activity_id))
            .where(VolunteerActivityLog.registration_id == registration_id)
        )
...
class VolunteerPointTransaction(BaseVolunteerPointTransaction, table=True):
    ...
    def select_leaderboard_period(
        ...
        query: Select = (
            ...
            .join(VolunteerActivityEvaluation, col(VolunteerActivityEvaluation.activity_id) == col(cls.activity_id))
        )
...
class UserActivityFullResponse(BaseModel):
    ...
    activity: VolunteerActivityLogPublic
    evaluation: Optional[VolunteerActivityEvaluationPublic] = None
